...
| 2026-07-05 | **Virtual Keys ohne `models`-Restriction** (`models = '{}'`): beide gregor-Keys (`opencode-gregor`, `catalog-proxy`) erben damit den **live Proxy-Katalog** statt einer hartcodierten Liste. Grund: LiteLLM `/v1/models` ist key-scoped (`get_available_models_for_user`) — eine Key-`models`-Liste pinnt den Katalog unabhängig von `config.yaml`/ollama (siehe PITFALLS.md). Leere Restriction = Katalog folgt automatisch dem Router. Für künftige User-Groups (Issue #6) werden dedizierte Keys mit expliziten `models` vergeben; der opencode/catalog-proxy-Default bleibt `{}`. | Beschlossen |
