...
## Current Focus
gregor-Stack live: LiteLLM + Postgres + SingleGpuGuard + models.dev-Proxy + whisper + Open WebUI in Docker. **qwen3:4b** (2,5 GB, 256K context) als sole LLM neben **whisper large-v3** (STT) — beide passen mit Headroom auf die 8 GB GPU. **Open WebUI** (:3000) mit LDAP-Auth gegen `ldap.spengergasse.at`, dient als user-facing Chat-Interface. Katalog = 2 Modelle (qwen3:4b, whisper-1). ollama re-enabled. Blob-Inventory `/opt/litellm/blob-inventory/` enthält 24 Blobs (40 GiB) aller 8 gemma4-Tags + qwen3:8b.
