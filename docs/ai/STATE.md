...
## Current Focus
gregor-Stack live: LiteLLM (DB-mode, `store_model_in_db=true`) + Postgres + SingleGpuGuard + models.dev-Proxy + whisper + Open WebUI in Docker. **5 LLM-Modelle** (qwen3.5:9b, qwen3.5:4b, qwen2.5:3b, qwen3:1.7b, llama3.2:3b) neben **whisper large-v3** (STT) — SingleGpuGuard swappt bei Modellwechsel. **Open WebUI** (:3000) mit LDAP-Auth gegen `ldap.spengergasse.at`, dient als user-facing Chat-Interface. Katalog = 6 Modelle. Alle Models outputen cleanen Text (kein Tool-JSON) dank `base_model_id=NULL`, `builtin_tools=false`, `supports_function_calling=false`, `tool_choice=none`, Modelfile system prompt. ollama re-enabled. **Issue #16 abgeschlossen (2026-09-17):** `10.8.0.18`-Hardcodings überall auf `10.8.0.16` (VPN-IP gregor) ersetzt, LiteLLM-DB auf 6 aktuelle Modelle synchronisiert.
